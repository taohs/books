...
spa.model = (function () {
    ...
    //    if the currrent user is not signed-in, an anonymous person
    //    object is returned;
    ...
    //    user name. The current user oject is changed to reflect.
    //    * logout() - revert the current user object to anonymous.
    //
    // jQuery global custom events published by the object includes:
    ...
    // Person objects provide the folowing methods:
    //    * get_is_user() - return true if object is the current user.
    //    * get_is_anon() - return true if object is anonymous.
    //
    // The attributes for a person object includes:
    //    * cid - string client id. This is always defined, and
    //    is only different from the id atrribute
    //    if the client data is not synced with the backed.
    //    * id - the unique id. This may be undefined if the object
    //    is not synced with the backend.
    //    * name -  the string name of the user.
    //    * css_map - a map of attributes used for avatar persentation.
    //
    // u
    ...
    completeLogin = function (user_list) {
        ...
        chat.join();// 让 completeLogin 方法调用 chat.join()，这样一旦用户完成登录就会自动加入聊天室
    ...
    makePerson = function (person_map) {
        ...
        if (cid === undefined || !name) {
            throw cid + 'client id and name required,' + cid + name;
        }
    ...
    people = (function () {
        ...
        logout = function () {
            var user = stateMap.user;                 // 移除is_moved变量
            chat._leave();// 让 people.logout 方法调用 chat._leave()；这样一旦用户完成登出就会自动退出聊天室
            // when we add chat, we should leave the chatroom here

            stateMap.user = stateMap.anon_user;
            clearPeopleDb();// 在登出时清除 Taffy 人员集合
