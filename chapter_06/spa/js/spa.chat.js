 ...
 *  2. 提供更换听者的方法
 *  3. 提供想其他人发送消息的方法
 *  4. 当听者不管是何原因而有变化的时候，发布一个事件。比如，假如听者下线了或者用户选择了新的听者。
 *  5. 当不管是何原因而需要更改消息框的时候，发布一个事件。比如，假如用户发送或者接受消息
 *  6. 当不管是何原因而导致在线人员列表发生变化，发布一个事件。比如，假如某人加入或者离开聊天室，或者任意用户移动了头像
 *
 * @API: 管理消息的传递和接收
 *  1.  维护听者的记录
 *  2. 发送消息的时候带上元数据（metadata） 信息，比如发送者的 ID，发送者的名字以及接收者的ID；
 *  3. 优雅的处理用户给离线人员发送的潜在情况
 *  4. 当从后端接收到消息时，发布 jQuery 全局事件，这样 jQuery 集合就可以订阅这些事件，并按照这些时间调用相应的函数
 *  
 */
